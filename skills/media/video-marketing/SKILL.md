...
**Terminal skill** — calls `higgsfieldcli generate` with `seedance_2_0` model directly. Does NOT delegate to `/video-skill`. This is the correct and intentional behavior for marketing video generation.

Marketing video generation hub. Takes user's product input + prompt, routes to the right reference, builds an enhanced prompt, and sends to Higgsfield CLI.
